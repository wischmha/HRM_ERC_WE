...
date:   "Written: 2023-04-12 to 2023-08-31 / Totally revised: 2024-11-07 to 2025-07-02 / Executed: `r Sys.Date()`"
output:
...
```{r fit_common_GAMs}
  ...
  observed <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    ...
    cbind(tmpc = df_grid$lag_0_3, age = m_age_sex$age[i], sex = m_age_sex$sex[i],
  ...
  response_functions <- rbind(observed %>% mutate(model = "Observed"), corrected %>% mutate(model = "Relative")) %>%
...
```{r heat_related_and_excess_mortality_calibration}
  ...
  # compute excess mortality as the difference between the observed mortality and the predicted mortality at the onset
  heat_related_excess_mortality <- predicted_mortality %>% filter(geo != "UKL") %>%
    mutate(heat_related_deaths = predicted_deaths - predicted_deaths_onset, excess_deaths = n_deaths - predicted_deaths_onset) %>%
