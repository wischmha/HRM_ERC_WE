...
date:   "Written: 2023-04-12 to 2023-08-31 / Totally revised: 2024-11-07 to 2025-04-15 / Executed: `r Sys.Date()`"
...
```{r setup, include = FALSE}
  ...
  rm(list = ls()); start_time <- proc.time()
  ...
  # stop all clusters (to return memory) and perform a full garbage collection
...
Expected execution time: Less than 15 minutes (12/24 physical/logical cores @ 5.7 GHz, 32 GB RAM)
...
```{r deaths}
  ...
  deaths_5y <- rbind(deaths_eurostat, deaths_portugal_2023, deaths_ONS, deaths_ONS_2023, deaths_INSEE) %>%
...
```{r population_distribution}
  ...
  rm(population_density, centroids, st_p, w_in); stop_gc()
```
...
```{r combine_geography_deaths_temperatures}
  ...
  rm(list = ls(pattern = "deaths|tmpc|temperatures|populat")); stop_gc()
  # data.frame(names = ls()) %>% mutate(size = map_int(names, ~object.size(get(.x)))) %>% arrange(desc(size))
```
...
```{r fit_individual_GAMs}
  ...
  df_summer %>% mutate(geo = gsub("UK", "EN.WA", str_sub(geo, 1, 2))) %>%
    group_by(geo, year) %>% reframe(n_deaths = sum(n_deaths)) %>% mutate(year = factor(year)) %>%
    pivot_wider(names_from = "geo", values_from = "n_deaths") %>%
    flextable() %>% set_caption(sprintf("Total number of deaths processed = %d", sum(df_summer$n_deaths))) %>% autofit()
...
```{r fit_common_GAMs}
  ...
  response_functions <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    ...
    lag_0_3 <- as.matrix(df_m %>% select(starts_with("tmpc_lag_0_3_dow")))
    gam2 <- bam(weekly_death_rate ~ 0 + geo + country:factor(year) + s(week, k = DOF_WEEK, bs = "ps", by = country) + s(lag_0_3, k = DOF_TMPC, bs = "ps"),
...
Compute and juxtapose heat-related and excess mortality by country by year, summed over regions, sex, and age groups. Exploratory calculation to assess plausibility of the model.
...
```{r heat_related_and_excess_mortality}
  ...
  # done, document elapsed and CPU time
