...
date:   "Written: 2023-04-12 to 2023-08-31 / Totally revised: 2024-11-07 to 2025-04-13 / Executed: `r Sys.Date()`"
...
```{r setup, include = FALSE}
  ...
  # create a clean slate, start the clock
  rm(list = ls())
  ...
  # load required libraries, after installing if necessary
  library_wrapper(c("tidyverse", "broom", "lubridate",
                    "readxl", "ncdf4", "sf", "ggh4x", "flextable", "ISOweek",
                    "mgcv", "scales", "splines", "foreach", "doParallel", "RhpcBLASctl"))
  ...
  plot_pdf_png <- function(file_name, aspect_ratio, plot_object, plot_width = 8.0, plot_res = 1200) {
    ...
    print(themed_object)
    ignore <- dev.off()
    knitr::include_graphics(sprintf("../Plots/%s.png", file_name), dpi = 1200)
  }
...
Prerequisites: A0_Download.sh successfully completed

Expected execution time: Less than 20 minutes (@ 12 physical cores, 24 logical cores, 32 GB RAM)
...
```
## Populations: Aggregate, Save, and Visualize
...
```{r populations}
  ...
  rm(list = setdiff(ls(pattern = "population"), c("population_4cat", "population_5y"))); gc(full = TRUE)
...
Deaths (No.) by Place of residence (NUTS - 2013), Sex and Age group; Weekly - Statistics Portugal, Deaths

Data for 2023 are based on the information registered in the Civil Register Offices until March 2024.

Manually defined query of INE database

https://www.ine.pt/xportal/xmain?xpid=INE&xpgid=ine_bdc_tree&contexto=bd&selTab=tab2

© Instituto Nacional de Estatístíca, Portugal [from Portal on April 12, 2025]

```{r deaths_portugal_2023}
  deaths_portugal_2023 <- read.csv("../Downloads/Population/Deaths_Portugal_NUTS2_2023.csv", sep = ";") %>%
    mutate(across(everything(), ~ifelse(.x == "", NA, .x))) %>%
    fill(c(week, geo, sex), .direction = "down") %>%
    mutate(age = str_replace_all(age, c(" years" = "", " or more" = "-99"))) %>%
    mutate(geo = paste("PT", str_sub(geo, 1, 2), sep = ""), country = "Portugal") %>%
    mutate(week = gsub(".. Week ", "-", week)) %>%
    separate_wider_delim(week, delim = "-", names = c("w", "y")) %>%
    mutate(date_from = ISOweek2date(sprintf("%04d-W%02d-1", as.integer(y), as.integer(w)))) %>%
    select(-w, -y)
```
## Deaths (DESTATIS)
...
```{r deaths}
  ...
  deaths_5y <- rbind(deaths_eurostat, deaths_portugal_2023, deaths_ONS, deaths_INSEE) %>%
  ...
  rm(list = setdiff(ls(pattern = "deaths"), c("deaths_4cat", "deaths_5y"))); gc(full = TRUE)
```
...
```{r population_distribution}
  ...
  plot_pdf_png("Figure_X2", aspect_ratio = 1.2,
    populated_grid %>%
  ...
  rm(population_density, centroids, st_p, w_in); stopImplicitCluster(); gc(full = TRUE)
```
...
```{r daily_mean_temperature_eobs}
  ...
  rm(nc_file, tmp); rm(list = setdiff(ls(pattern = "temperature"), c("temperatures_daily_lagged", "temperatures_weekly_lagged"))); stopImplicitCluster(); gc(full = TRUE)
  ...
  plot_pdf_png("Figure_X3", aspect_ratio = 0.5,
    consecutive_days_25 %>%
    ggplot(aes(x = length, y = n, group_by = country, color = country, label = country)) +
      ...
      labs(y = "Total Number of Warm Stretches (Sum over Regions)", x = "Consecutive Days >= 25 °C Mean Temperature") +
      std_theme() + theme(legend.position = "bottom", legend.title = element_blank()))
...
```{r temperature_plots}
  ...
  plot_pdf_png("Figure_X4", aspect_ratio = 0.5,
  ...
  print("Figure_X4: Average Temperatures", fill = "T [°C]")
```
...
```{r deaths_plots}
  ...
  plot_pdf_png("Figure_X6", aspect_ratio = 1.2,
    ...
    ggplot(aes(x = date_from, y = n_deaths_std, group_by = country, color = country)) +
      ...
      std_theme() + theme(legend.position = "top", axis.ticks.x=element_blank()) +
      guides(color = guide_legend(nrow = 2, byrow = FALSE)) + facet_wrap(~panel, ncol = 1, scales = "free_x"))
  print("Figure_X6: Deaths (EU27 Standard Population)")
```
...
```{r exposure_response_plot}
  ...
  plot_pdf_png("Figure_2", aspect_ratio = 1.0,
    ...
    ggplot(aes(x = tmpc_max_lag_0_3, y = r_deaths, group_by = interaction(age, decade), color = age, weight = n_pop)) +
      ...
      guides(fill = guide_legend(nrow = 2, byrow = FALSE), linetype = guide_legend(nrow = 2, byrow = FALSE, override.aes = list(color = "#7F7F7F"))) +
...
```{r combine_geography_deaths_temperatures}
  ...
  rm(deaths_5y, deaths_4cat); stopImplicitCluster(); gc(full = TRUE)
```
## Pass 1 - Individual models by region, by age, by sex, by year: Onset of heat-related risk

Individual GAMs are computed for the weekly death rates (assuming a gaussian error distribution) as a function of the mean temperature across the prior 0..3 days, separately by region(geo), by age groups, by sex, and by year. The onset (t_zero) for positive heat-related mortaliy risk is determined for each model (along with the slope (s_zero) at this point),  aggregated across all available years, and subtracted from the actual, observed temperatures, by region(geo), by age, by sex. Many individual models do not show a minimum (t_min), so that the computed estimates for the minima would be less robust and can thus not serve instead as the temperature reference. As the data frame for each model contains only 26 data points (LAST_SUMMER_WEEK - FIRST_SUMMER_WEEK + 1), the seasonal terms are modeled using a smooth function with only 5 DoF. Data from two COVID years (2020 and 2022) are excluded to avoid potential confounding by pandemic mortality effects during summer.

```{r fit_individual_GAMs}
  # reload summer data file (entry point to restart processing)
  load(file = "../Processed/DeathsPopTemp.Rdata")
  df_summer %>% mutate(geo = gsub("UK", "EN.WA", str_sub(geo, 1, 2))) %>% group_by(geo, year) %>% reframe(n_deaths = sum(n_deaths)) %>% mutate(year = factor(year)) %>%
    pivot_wider(names_from = "geo", values_from = "n_deaths") %>% flextable() %>% set_caption(sprintf("Total number of deaths processed = %d", sum(df_summer$n_deaths))) %>% autofit()
  ...
  # fit separate models either by region, age group, sex, and year (1st pass)
  config_parallel(max_threads = 6)
  df_tmpc <- foreach (i = 1:nrow(m_geo_age_sex_year), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    ...
    gam1 <- bam(weekly_death_rate ~ 1 + s(week, k = DOF_WEEK, bs = "ps") + s(lag_0_3, k = DOF_TMPC, bs = "ps"), data = df_m, family = gaussian, discrete = TRUE)
  ...
  plot_pdf_png("Figure_X7", aspect_ratio = 0.5,
  ...
  tidy(lm_zero, conf.int = TRUE) %>% filter(grepl(":year", term)) %>% arrange(desc(estimate)) %>%
    mutate(Trend = sprintf("%.02f [%.02f, %.02f]", 10.0 * estimate, 10.0 * conf.low, 10.0 * conf.high)) %>%
  ...
  tidy(lm_sens, conf.int = TRUE) %>% filter(grepl(":year", term)) %>% arrange(estimate) %>%
    mutate(Trend = sprintf("%.03f [%.03f, %.03f]", 10.0 * estimate, 10.0 * conf.low, 10.0 * conf.high)) %>%
    mutate(term = str_replace_all(term, c("country" = "", ":year" = ""))) %>%
  ...
  plot_pdf_png("Figure_S1", aspect_ratio = 0.5,
    df_tzero_tmin %>% group_by(geo, sex) %>% mutate(t_zero = t_zero - max(t_zero)) %>% ungroup() %>%
    left_join(., geo_info, join_by("geo" == "id_region")) %>% st_as_sf() %>%
    ggplot() +
      geom_sf(aes(fill = t_zero)) +
      labs(fill = "T-Tmax [°C]", y = "© EuroGeographics for the administrative boundaries") +
  ...
  plot_pdf_png("Figure_X8", aspect_ratio = 0.5,
    df_tzero_tmin %>%
    left_join(., geo_info, join_by("geo" == "id_region")) %>% st_as_sf() %>%
    ggplot() +
...
```{r fit_common_GAMs}
  ...
  response_functions <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    ...
    lag_0_3 <- as.matrix(df_m %>% select(starts_with("tmpc_lag_0_3_dow")))
    gam2 <- bam(weekly_death_rate ~ geo + country:factor(year) + s(week, k = DOF_WEEK, bs = "ps", by = country) + s(lag_0_3, k = DOF_TMPC, bs = "ps"),
    ...
    observed <- cbind(tmpc = df_grid$lag_0_3,
    ...
    # response functions, by age and sex, to be aggregated
    corrected <- cbind(tmpc = df_grid$lag_0_3,
          as.data.frame(predicted_terms$fit   ) %>% rename_with(~str_replace_all(.x, c("s[(]lag_" = "response_", "[)]" = ""))),
          as.data.frame(predicted_terms$se.fit) %>% rename_with(~str_replace_all(.x, c("s[(]lag_" = "se_", "[)]" = ""))))
  ...
  plot_pdf_png("Figure_4", aspect_ratio = 0.5,
    response_functions %>% arrange(desc(age), sex) %>% mutate(age_sex = fct_inorder(paste(age, sex, sep = ":"))) %>%
      ggplot(aes(x = tmpc, y = response, group_by = age_sex, fill = age_sex, color = age_sex)) +
        ...
        geom_line() +
        labs(y = "Weekly Mortality Rate [1/100.000], with 95%-CI", x = "        Temperature T [°C]                                               Relative Temperature T-T0 [°C]") +
        ...
        std_theme() + facet_wrap(~model, nrow = 1, scales = "free_x"))
  ...
  power_laws <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    nls1 <- nls(response ~ a + b * tmpc^c, start = list(a = 0.0, b = 0.1, c = 1.5),
      data = response_functions %>% filter(model == "Temperature above Onset", tmpc > 0.0, age == m_age_sex$age[i], sex == m_age_sex$sex[i]))
    data.frame(power = coef(nls1)["c"], power_cl = confint(nls1, "c", level = 1.0 - ALPHA)[1], power_ch = confint(nls1, "c", level = 1.0 - ALPHA)[2]) %>%
      mutate(Age = m_age_sex$age[i], Sex = m_age_sex$sex[i])
  }
  power_laws %>% mutate(power_ci = sprintf("%.02f [%.02f, %.02f]", power, power_cl, power_ch)) %>%
    select(Age, Sex, Power = power_ci) %>% arrange(Age, Sex) %>%
...
```{r fit_loading_common_models}
  ...
  load_functions <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv", "splines")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer_shifted %>% filter(!year %in% COVID_YEARS, age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop, year = year - LOADS_YEAR) # reference year for factor comparison plot
    ...
    gam3 <- bam(weekly_death_rate ~ 0 + geo + country:factor(year) + s(week, k = DOF_WEEK, bs = "ps", by = country) + s(lag_0_3, k = DOF_TMPC, bs = "ps"),
                weights = n_pop, data = df_m, family = gaussian, discrete = TRUE)

    # append response functions, cancelling out the response below the risk onset
    df_m <- df_m %>%
      mutate(t_response = predict.gam(gam3, newdata = list(geo = df_m$geo, country = df_m$country, year = df_m$year, week = df_m$week, lag_0_3 = lag_0_3)) -
                          predict.gam(gam3, newdata = list(geo = df_m$geo, country = df_m$country, year = df_m$year, week = df_m$week, lag_0_3 = lag_0_3_clipped)))
    ...
    lm3 <- lm(weekly_death_rate ~ 0 + geo + country:factor(year) + country:bs(week, df = DOF_WEEK) + offset(t_response) + country:t_response + country:year:t_response,
  ...
  results %>% arrange(country, age, sex) %>%
    mutate(value = ifelse(Type == "Scale", value + 1.0, value)) %>%
    mutate(value = sprintf("%.02f (%.02f to %.02f) %s", value, value + qnorm(ALPHA * 0.5) * SE, value + qnorm(1.0 - ALPHA * 0.5) * SE, ifelse(p.value < ALPHA, " *", ""))) %>%
    select(Type, Country = country, Age = age, Sex = sex, value) %>%
    pivot_wider(names_from = Type, values_from = value) %>%
    flextable() %>%
      set_caption("Heat Sensitivity: Scale (* != 1) and Trend (* != 0) by Country") %>%
      hline(i = seq(8, 72, 8)) %>% align(align = "center", part = "all") %>% align(j = 1, align = "left", part = "all") %>% autofit()
  ...
  plot_pdf_png("Figure_5", aspect_ratio = 0.7,
    ...
    ggplot() +
      ...
      labs(fill = "Scale", x = "", y = "© EuroGeographics for the administrative boundaries") +
  ...
  plot_pdf_png("Figure_6", aspect_ratio = 0.7,
    ...
    ggplot() +
      ...
      labs(fill = "Annual\nScale\nChange\n[1/year]", x = "", y = "© EuroGeographics for the administrative boundaries") +
      std_theme() + theme(legend.position = "right", legend.text = element_text(hjust = 1)) + facet_grid(vars(sex), vars(age)))
  print(sprintf("Figure_6: Heat Sensitivity Trend (%d-%d): Slope = Annual Change to Factor", min(YEARS_MODEL), max(YEARS_MODEL)))
```
## Pass 4 - Heat-Related and Excess Mortality

Compute and juxtapose heat-related and excess mortality by country by year, summed over regions, sex, and age groups. Exploratory check only, for the appendix.

```{r heat_related_and_excess_mortality}
  # fit common models by sex and age group, across regions and years
  heat_related_excess_mortality <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv", "splines")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer_shifted %>% filter(!year %in% COVID_YEARS, age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop)
    ...
    lag_0_3         <- as.matrix(df_m %>% select(starts_with("tmpc_lag_0_3_dow")))
    lag_0_3_clipped <- as.matrix(df_m %>% select(starts_with("tmpc_lag_0_3_dow")) %>% mutate(across(everything(), ~pmin(0.0, .))))
    gam4 <- bam(weekly_death_rate ~ 0 + geo + country:factor(year) + s(week, k = DOF_WEEK, bs = "ps", by = country) + s(lag_0_3, k = DOF_TMPC, bs = "ps"),
                weights = n_pop, data = df_m, family = gaussian, discrete = TRUE)

    # append response functions, cancelling out the response below the risk onset
    df_m <- df_m %>%
      mutate(t_response = predict.gam(gam4, newdata = list(geo = df_m$geo, country = df_m$country, year = df_m$year, week = df_m$week, lag_0_3 = lag_0_3)) -
                          predict.gam(gam4, newdata = list(geo = df_m$geo, country = df_m$country, year = df_m$year, week = df_m$week, lag_0_3 = lag_0_3_clipped)))

    # extract load functions from a linear model, using the common response as a predictor variable, replacing s(week) by bs(week)
    # and computing country level deviations from the mean response, specified as the offset
    lm4 <- lm(weekly_death_rate ~ 0 + geo + country:factor(year) + country:bs(week, df = DOF_WEEK) + offset(t_response) + country:factor(year):t_response,
              weights = n_pop, data = df_m)

    # compute heat-related mortality as the difference between the predicted mortality for observed temperatures and the predicted mortality at the max of the minima
    # compute excess mortality as the difference between the observed mortality and the predicted mortality at the max of the minima
    df_m <- df_m %>%
      mutate(predicted_deaths       = n_pop * 1.0e-5 * predict(lm4, type = "response"),
             predicted_deaths_onset = n_pop * 1.0e-5 * predict(lm4, type = "response", newdata = df_m %>% mutate(t_response = 0.0))) %>%
      mutate(heat_related_deaths    = round(predicted_deaths - predicted_deaths_onset, 0),
             excess_deaths          = round(n_deaths         - predicted_deaths_onset, 0))

    # return aggregated numbers, summed over regions and weeks, by country, year, sex, and age group, but exclude Wales
    df_m %>% filter(geo != "UKL") %>%
      group_by(country, year) %>% reframe(heat_related_deaths = sum(heat_related_deaths), excess_deaths = sum(excess_deaths)) %>%
      mutate(age = m_age_sex$age[i], sex = m_age_sex$sex[i])
  }

  # create table of heat-related and excess mortality
  heat_related_excess_mortality %>% group_by(Country = country, Year = year) %>% reframe(HR_EX = sprintf("%.0f/%.0f", sum(heat_related_deaths), sum(excess_deaths))) %>%
