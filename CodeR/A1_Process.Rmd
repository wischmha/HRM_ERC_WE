...
date:   "Written: 2023-04-12 to 2023-08-31 / Totally revised: 2024-11-07 to 2024-12-24 / Executed: `r Sys.Date()`"
...
```{r exposure_response_plot}
  ...
  print("Raw Exposure Response Curves (Summer), Incremental Deaths")
  ...
  plot_pdf_png("Figure_2", aspect_ratio = 1.0,
    ...
    ggplot(aes(x = tmpc_max_lag_0_3, y = r_deaths, group_by = legend, color = legend, weight = n_pop)) +
      geom_smooth(aes(fill = legend), formula = y ~ x, method = "loess", span = 0.8, se = TRUE, alpha = 0.2) +
      labs(color = "", fill = "",
           x = "Exposure: Mean Temperature over prior 0:3 Days, Maximum over 7 Days of Week",
           y = "Weekly Rate of Incremental Deaths in Summer [1/100k]\nvs. Region-Year-Sex-Age Mean, All Regions") +
      scale_x_continuous(breaks = seq(10, 30, 10), labels = ~sprintf("%d °C", .x)) +
  ...
  print("Raw Exposure Response Curves (Summer), Incremental Deaths, 74-84 and 85+ Years")
```
...
```{r fit_individual_GAMs}
  ...
  # fit separate models either by region, age group, sex, and year (1st pass, excluding years with COVID) or by age group and sex
  ...
  df_tmpc <- foreach (i = 1:nrow(m_geo_age_sex_year), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    ...
    cbind(df_grid %>% filter(response <= 0) %>% reframe(t_zero = max(tmpc)),
          ...
          m_geo_age_sex_year[i,], n_pop = df_m$n_pop)
  }
  ...
  as.data.frame(summary(lm_zero)$coef) %>% mutate(Country = rownames(.)) %>% filter(grepl(":year", Country)) %>%
    ...
    mutate(Trend = sprintf("%.2f [%.2f, %.2f]", Estimate, Low, High)) %>%
    mutate(Country = str_replace_all(Country, c("country" = "", ":year" = ""))) %>%
    select(Country, "Trend [C°/10 years]" = Trend) %>%
  ...
  df_tzero_tmin <- m_geo_age_sex_year %>%
  ...
  df_tzero_tmin %>%
    ...
    flextable() %>%
      set_caption(sprintf("Onset of Heat-Related Risk, by Country, Sex, and Age Group (Median across Regions, Linear Regression across Years)")) %>%
  ...
  df_tzero_tmin %>%
    ...
    pivot_wider(values_from = t_zero, names_from = age_sex) %>%
    flextable() %>%
      set_caption(sprintf("Onset of Heat-Related Risk, by Region, Sex, and Age Group (Linear Regression across Years)")) %>%
  ...
  # shift onset of heat-related risk to 0 °C by region, by age, by sex, with the most recent year as the fixed reference (2023), save
...
```{r fit_common_GAMs}
  # fit common models by sex and age group, across regions and years, removing years with COVID
  ...
  response_functions <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer %>% filter(age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop) %>% filter(!year %in% COVID_YEARS)
    ...
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer_shifted %>% filter(age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop) %>% filter(!year %in% COVID_YEARS)
...
```{r fit_loading_common_models}
  # fit common models by sex and age group, across regions and years, removing years with COVID
  load_functions <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer_shifted %>% filter(age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop) %>% filter(!year %in% COVID_YEARS) %>%
  ...
  results %>% arrange(country, age, sex) %>%
    ...
    mutate(value = sprintf("%.2f (%.2f to %.2f) %s", value, value + qnorm(ALPHA * 0.5) * SE, value + qnorm(1.0 - ALPHA * 0.5) * SE, ifelse(p.value < ALPHA, " *", ""))) %>%
  ...
  print(sprintf("Heat Sensitivity (%d): Scale x Common Age-Sex Response (across Countries and %d-%d)", LOADS_YEAR, min(YEARS_MODEL), max(YEARS_MODEL)))
  ...
  plot_pdf_png("Figure_6", aspect_ratio = 0.7,
    ...
    ggplot() +
      ...
      std_theme() + theme(legend.position = "right", legend.text = element_text(hjust = 1)) + facet_grid(vars(sex), vars(age)))
  print(sprintf("Heat Sensitivity Trend (%d-%d): Slope = Annual Change to Factor", min(YEARS_MODEL), max(YEARS_MODEL)))
```
...
```{r heat_related_and_excess_mortality}
  # fit common models by sex and age group, across regions and years, removing years with COVID
  heat_related_excess_mortality <- foreach (i = 1:nrow(m_age_sex), .combine = "rbind", .packages = c("tidyverse", "mgcv")) %dopar% {
    # extract subset of data for region, calculate weekly_death_rate for 100k population
    df_m <- df_summer_shifted %>% filter(age == m_age_sex$age[i], sex == m_age_sex$sex[i]) %>%
      mutate(weekly_death_rate = 1.0e5 * n_deaths / n_pop) %>% filter(!year %in% COVID_YEARS)
    ...
    # extract load functions from a linear model, using the common response as a predictor variable, replacing s(week) by bs(week)
    # and computing country level deviations from the mean response, specified as the offset
    lm4 <- lm(weekly_death_rate ~ 0 + geo + country:factor(year) + country:bs(week, df = DOF_WEEK) + offset(t_response) + country:t_response + country:factor(year):t_response,
              weights = n_pop, data = df_m)
    ...
    # return aggregated numbers, summed over regions and weeks, by country, year, sex, and age group
    df_m %>% group_by(country, year) %>% reframe(heat_related_deaths = sum(heat_related_deaths), excess_deaths = sum(excess_deaths)) %>%
      mutate(age = m_age_sex$age[i], sex = m_age_sex$sex[i])
  }
